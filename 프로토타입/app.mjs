...
import {track,newSession} from './track.mjs';
...
function act(action){
 ...
 else if(action==='restart'){forwardStates=[];clearStage2();research?.log('restart');newSession();research?.reset();state=freshRun();}
 ...
 if(state.phase==='result'){const r=result(state);track('stage1_result',{ending:state.ending,cash:state.cash,shares:state.shares,health:state.health,spent:state.spent,equity:r.equity,profit:r.profit,history:state.history},{stage:1,turn:state.turn});}
...
function restartCampaign(){forwardStates=[];stage2Active=false;newSession();state=freshRun();research?.reset();save();render();}
